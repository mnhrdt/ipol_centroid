...
\ipolSetTitle{The centroid method for the correction of turbulence}
\ipolSetAuthors{
...
field~$\u$ is a vector field~$\v$ such that if
\[
\varphi(\x):=\x+\u(\x)
\]
then
...
is~$\y=\v(\x)$.  The Lipschitz constant of this mapping is the operator norm of
the matrix~$D\u$, which can be roughly bounded
by~$|\mathrm{div}(\u)|$.  Thus, for the pixels~$\x$
where~$|\mathrm{div}(\u)(\x)|<1$ the fixed-point iterations converge to the
...
interpolation~\cite{keys1981cubic}, but this can be replaced by a higher-order
spline.
...
Efros et~al.~\cite{efros2004} and Tian et~al.~\cite{tian2009seeing}.  We also
use synthetic sequences produced by deforming a color photograph by smooth
random fields.

Figures~\ref{fig:expcvv} through~\ref{fig:expsim} show some results for these
sequences.  See the on-line demo associated to this article for a larger
collection of experiments.  Notice that in general the method is always a
clear improvement over the average image.
...
\begin{figure}[p]
	...
	reference~\cite{efros2004} (used with permission).
...
\begin{algorithm}[H]
	...
	\For{$i=1,\ldots,5$}{
		...
		{\displaystyle\sum_{n=1}^N \x_n /\sqrt{\varepsilon^2+\left\|\y-\x_n\right\|^2}}
