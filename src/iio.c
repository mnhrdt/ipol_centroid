...
//
// Note: this file was copied from the official distribution of the IIO library
// by Enric Meinhardt-Llopis.  Licence information, documentation and eventual
// newer versions can be downloaded from this address:
//
...
static FILE *xfopen(const char *s, const char *p)
{
	...
	f = fopen(s, p);
	if (f == NULL)
		fail("can not open file \"%s\" in mode \"%s\"", s, p);
